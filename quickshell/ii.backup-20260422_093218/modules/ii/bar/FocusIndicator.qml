...
Item {
    // Click-to-pause removed as requested
    ...
    function updateCountdown(endTime) {
        ...
        if (remaining <= 0) {
            root.countdown = "0s";
            if (root.focusActive && !root.paused)
                checkTimerTrigger.running = true;
    ...
    Process {
        id: checkTimerTrigger

        command: ["/bin/bash", "/home/pratik/.config/hypr/custom/scripts/focus-mode.sh", "--check-timer"]
    }
